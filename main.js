...
const confirm_regis_password_wrong = document.querySelector('.register-wrong-pass-confirm');
const passw = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{6,30}$/;
...
const check_confirmRegist_pass = event => {
    const regis_password=document.querySelector('.regis-pass').value;
    const confirm_regis_password=document.querySelector('.confirm-regis-pass').value;

    if (confirm_regis_password!=regis_password && confirm_regis_password.length!=0){
        confirm_regis_password_wrong.style.display = 'inline-block';
    } else {
        confirm_regis_password_wrong.style.display = 'none';
    } 
}
const regis_debouncedKeyUp_confirmPass = debounce(check_confirmRegist_pass, 500); 
